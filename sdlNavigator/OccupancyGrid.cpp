...
void OccupancyGrid::gro(int gridX, int gridY)
{
  ...
  //printf("grid[%d][%d].himm=%.2f\n",gridX,gridY,newValue);

  //grid[gridX][gridY].himm =
  //double newValue = \
  //  /* First row. */
  //  (((gridX == 0) || (gridY == 0)) ? 0 : grid[gridX - 1][gridY - 1].himm * 0.5) + \
  //  ((gridY == 0) ? 0 : grid[gridX][gridY - 1].himm * 0.5) + \
  //  (((gridX == WINDOW_SIZE_X) || (gridY == 0)) ? 0 : grid[gridX + 1][gridY - 1].himm * 0.5) + \
  //  /* Second row. */
  //  ((gridX == 0) ? 0 : grid[gridX - 1][gridY].himm * 0.5) + \
  //  grid[gridX][gridY].himm + \
  //  (gridX == WINDOW_SIZE_X) ? 0 : grid[gridX + 1][gridY].himm * 0.5 + \
  //  /* Third row. */
  //  (((gridX == 0) || (gridY == WINDOW_SIZE_Y)) ? 0 : grid[gridX - 1][gridY + 1].himm * 0.5) + \
  //  ((gridY == WINDOW_SIZE_Y) ? 0 : grid[gridX][gridY + 1].himm * 0.5) + \
  //  (((gridX == WINDOW_SIZE_X) || (gridY == WINDOW_SIZE_Y)) ? 0 : grid[gridX + 1][gridY + 1].himm * 0.5);
